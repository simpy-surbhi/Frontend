...
export class AdminHomeComponent implements OnInit {
  ...
  constructor(
    ...
    private choicesservice: McqchoicesService
  ) {
    this.getAll();
  }
  ...
  editEpita(elementId: string) {
    let exam: Exam = this.ItemsArray.filter(
      (x) => x.id === Number(elementId)
    )[0];
  ...
  deleteQuestion(elementId: string) {
    ...
    let question: Question = this.questions.filter(
      (x) => x.id === Number(elementId)
    )[0];
    console.log(question);
    this.questionService.post(question, (question: Question) => {
      let questionlocal: Question = this.questions.filter(
        (x) => x.id === question.id
      )[0];
  ...
  getAllQuestion() {
    var edit = document.querySelector('.addbutton');
